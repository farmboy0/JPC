    ...
    * and removes itself when destroyed. */
    public static class MixerObject {
        ...
        public MixerChannel Install(MIXER_Handler handler, /*Bitu*/int freq, String name) {
            if (!installed) {
                if (name.length() > 31)
                    throw new IllegalStateException("Too long mixer channel name");
                m_name = name;
                installed = true;
    ...
    static boolean MIXER_CallBack(byte[] stream, int len) {
        ...
        if (mixer.done < need) {
            ...
            if (left < mixer.min_needed) {
                ...
                if (LOG_BUFFERS)
                    Log.log(Level.INFO,
                        String.format("needed underrun need %d, have %d, min %d, left %d", need, mixer.done, mixer.min_needed, left));
                reduce = need - left;
                index_add = (reduce << MIXER_SHIFT) / need;
            } else {
                reduce = need;
