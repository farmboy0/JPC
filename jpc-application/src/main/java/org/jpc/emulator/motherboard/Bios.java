    ...
    private void load(PhysicalAddressSpace addressSpace) {
        ...
        if (this instanceof SystemBIOS) {
            ...
            for (; endAddress >= -imageData.length; endAddress -= AddressSpace.BLOCK_SIZE, imageOffset -= AddressSpace.BLOCK_SIZE, lowAddress -= AddressSpace.BLOCK_SIZE) {
                EPROMMemory eprom = new EPROMMemory(AddressSpace.BLOCK_SIZE, 0, imageData, imageOffset, AddressSpace.BLOCK_SIZE,
            ...
            EPROMMemory ep = new EPROMMemory(AddressSpace.BLOCK_SIZE, loadAddress & AddressSpace.BLOCK_MASK, imageData, 0,
                nextBlockStart - loadAddress, addressSpace.getCodeBlockManager());
            ...
            if (imageOffset < imageData.length) {
                ep = new EPROMMemory(AddressSpace.BLOCK_SIZE, 0, imageData, imageOffset, imageData.length - imageOffset,
                    addressSpace.getCodeBlockManager());
                shadow = new ShadowEPROMMemory(AddressSpace.BLOCK_SIZE, ep, addressSpace.getCodeBlockManager());
                addressSpace.mapMemory(epromOffset, shadow);
            }
