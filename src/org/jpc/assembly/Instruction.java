...
public class Instruction {
    ...
    private static String getOperandType(ZygoteOperand op, Operand val) {
        ...
        if (op.name.equals("Ev")) {
            if (val.size == 16)
                ...
                throw new IllegalStateException("Unknown Ev size: " + val.size);
        }
        if (op.name.equals("Gv") || op.name.equals("Gz")) {
            if (val.size == 16)
                ...
                throw new IllegalStateException("Unknown Gv/z size: " + val.size);
        }
        if (op.name.equals("Iv") || op.name.equals("Iz")) {
            if (val.size == 16)
                ...
                throw new IllegalStateException("Unknown Iv/z size: " + val.size);
        }
        if (op.name.equals("Jz")) {
            if (val.size == 16)
                ...
                throw new IllegalStateException("Unknown Jv size: " + val.size);
        }
        if (op.name.equals("Ov")) {
            if (val.size == 16)
                ...
                throw new IllegalStateException("Unknown Ov size: " + val.size);
        }
    ...
    public String toString(boolean pattern) {
        ...
        if (print_size) {
            if (operand.length == 1)
                return String.format("(%d bytes) %s%s %s", x86Length, pfx, operator + (branch_dist == null ? "" : " " + branch_dist),
                    operand[0].toString(pattern));
            else if (operand.length == 2)
                return String.format("(%d bytes) %s%s %s, %s", x86Length, pfx, operator + (branch_dist == null ? "" : " " + branch_dist),
                    operand[0].toString(pattern), operand[1].toString(pattern));
            else if (operand.length == 3)
                return String.format("(%d bytes) %s%s %s, %s, %s", x86Length, pfx,
                    operator + (branch_dist == null ? "" : " " + branch_dist), operand[0].toString(pattern), operand[1].toString(pattern),
                    operand[2].toString(pattern));
        ...
        if (operand.length == 1)
            ...
            return String.format("%s%s %s,%s", pfx, operator /*+ (branch_dist == null ? "": " "+branch_dist)*/,
                operand[0].toString(pattern), operand[1].toString(pattern));
        else if (operand.length == 3)
            return String.format("%s%s %s,%s,%s", pfx, operator /*+ (branch_dist == null ? "": " "+branch_dist)*/,
                operand[0].toString(pattern), operand[1].toString(pattern), operand[2].toString(pattern));
        return String.format("%s%s", pfx, operator /*+ (branch_dist == null ? "": " "+branch_dist)*/);
    }

    public boolean isBranch() {
        return jcc.contains(operator) || jmp.contains(operator) || call.contains(operator) || ret.contains(operator)
            || hlt.contains(operator) || usesControlReg(0) || operator.equals("int");
    }
