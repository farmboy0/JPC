 ...
 * @see <a href="http://pdos.csail.mit.edu/6.828/2007/readings/hardware/8237A.pdf"> 8237A -
 * Datasheet</a>
 * @author Chris Dennis
 */
public class DMAController extends AbstractHardwareComponent implements IODevice {
    ...
    private static final int CMD_NOT_SUPPORTED = COMMAND_MEMORY_TO_MEMORY | COMMAND_ADDRESS_HOLD | COMMAND_COMPRESSED_TIMING
        | COMMAND_CYCLIC_PRIORITY | COMMAND_EXTENDED_WRITE | COMMAND_DREQ_SENSE_LOW | COMMAND_DACK_SENSE_LOW;
    ...
    public class DMAChannel implements Hibernatable {
         ...
         * <p>
         * Allows a <code>DMATransferCapable</code> device to read the section of memory currently pointed
         * to by this channels internal registers.
         ...
         * <p>
         * Allows a <code>DMATransferCapable</code> device to write to the section of memory currently
         * pointed to by this channels internal registers.
    ...
    /**
     * Constructs a DMA controller (either primary or secondary). If <code>highPageEnable</code> is true
     * then 32 bit addressing is possible, otherwise the controller is limited to 24 bits.
     ...
     * <p>
     * Non-primary or secondary controllers operate by being chained off the primary controller.
    ...
    /**
     * Register the given <code>DMATransferCapable</code> device with the specified channel.
     * <p>
     * Subsequent DMA requests on this channel will call the <code>handleTransfer</code> method on
     * <code>device</code>.
     * @param channel channel index.
