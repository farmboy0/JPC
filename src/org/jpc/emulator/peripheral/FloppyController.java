        ...
        if (dmaEnabled) {
            ...
            if ((direction == DIRECTION_SCANE || direction == DIRECTION_SCANL || direction == DIRECTION_SCANH) && dmaMode == 0
                ...
                if (Option.useBochs.isSet())
                    clock.newTimer(new DMATimer())
                        .setExpiry(clock.getEmulatedNanos() + 1000 * (200000 / drive.sectorCount)
                            + 1000000000 / clock.getTickRate()/*make it trigger the instruction after this (round up)*/);
                else
                    dma.holdDmaRequest(DMA_CHANNEL & 3);
                return;
            } else
                LOGGING.log(Level.INFO, "DMA mode {0,number,integer}, direction {1,number,integer}",
                    new Object[] { Integer.valueOf(dmaMode), Integer.valueOf(direction) });
        }
