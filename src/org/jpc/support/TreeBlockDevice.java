...
public class TreeBlockDevice implements BlockDevice {
    ...
    */
     ...
     * <p>
     * If <code>buffer</code> is true then writes to the drive are buffered in memory. Otherwise they
     * are commited back to the local filesystem.
    ...
    private int followFatChainLink(int cluster) {
        ...
        if (fatOffset < fatImage.length)
            return (fatImage[fatOffset] & 0xFF) + ((fatImage[fatOffset + 1] & 0xFF) << 8) + ((fatImage[fatOffset + 2] & 0xFF) << 16)
                + ((fatImage[fatOffset + 3] & 0xFF) << 24);
        else
            return 0;
    }
    ...
    public int read(long sectorNumber, byte[] buffer, int size) {
        ...
        if (entry != null)
            ...
            System.arraycopy(fatImage, (int)((sectorNumber - HEADER_SECTION_LENGTH) % fatSize) * SECTOR_SIZE, buffer, 0,
                size * SECTOR_SIZE);
        else
    ...
    private void readToWrite(Map hashFAT, int startingCluster, File file) //make this return an int to signify success
    {
        ...
        if (file.isDirectory()) {
            ...
            for (int k = 0; k < dir.length / 32; k++) {
                ...
                newStartCluster = (dir[32 * k + 26] & 0xFF) + ((dir[32 * k + 27] & 0xFF) << 8) + ((dir[32 * k + 20] & 0xFF) << 16)
                    + ((dir[32 * k + 21] & 0xFF) << 24);
