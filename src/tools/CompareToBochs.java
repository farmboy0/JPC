...
public class CompareToBochs {
    ...
    public static final String[] win311_crash = {
        "-hda",
        "caching:64MBDOS5WFW311.img",
        "-boot",
        "hda",
        "-ips",
        "1193181",
...
//            }
            if (fast[16] > nextMilestone) {
                System.out.printf("Reached %x ticks! Averaging %d IPS\n", fast[16],
                    fast[16] * (long)1000000000 / (System.nanoTime() - startTime));
...
//            return false;
    ...
    public static void printPage(byte[] fast, byte[] old, int esp) {
        ...
        for (int i = 0; i < 1 << 12; i++) {
            ...
            byte b2 = old[i];
            if (b1 != b2) {
                System.out.println("Memory not the same at 0x" + Integer.toHexString(address + i) + ", values: "
                    + Integer.toHexString(b1 & 0xff) + " " + Integer.toHexString(b2 & 0xff));
            }
        }
    }
