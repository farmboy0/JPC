...
public class OracleFuzzer {
    ...
    public static int[] getCanonicalRealModeInput(int codeEIP, boolean random) {
        ...
        if (random) {
            ...
            inputState[9] = (r.nextBoolean() ? 0 : EmulatorControl.OF_FLAG) | (r.nextBoolean() ? 0 : EmulatorControl.SF_FLAG)
                | (r.nextBoolean() ? 0 : EmulatorControl.ZF_FLAG) | (r.nextBoolean() ? 0 : EmulatorControl.AF_FLAG)
                | (r.nextBoolean() ? 0 : EmulatorControl.PF_FLAG) | (r.nextBoolean() ? 0 : EmulatorControl.CF_FLAG);
            for (int i = 10; i < 16; i++)
            ...
            inputState[10] = 0x3000;
            inputState[30] = inputState[10] << 4; // es
            inputState[11] = 0x0;
            inputState[31] = inputState[11] << 4; // cs
            inputState[12] = 0x4000;
            inputState[32] = inputState[12] << 4; // ds
            inputState[13] = 0x5000;
            inputState[33] = inputState[13] << 4;// ss
            inputState[14] = 0x6000;
            inputState[34] = inputState[14] << 4;// fs
            inputState[15] = 0x7000;
            inputState[35] = inputState[15] << 4;// gs
    ...
    public static void testRange(int codeEIP, byte[][] prefices, int[] inputState, int mode, boolean is32Bit, BufferedWriter cases,
    ...
    private static int testOpcode(int currentCSEIP, byte[] code, int x86, int[] inputState, int flagMask, int mode, boolean is32Bit,
        Map<Integer, byte[]> mem, BufferedWriter cases, BufferedWriter log) throws IOException {
    ...
    private static void printCase(byte[] code, int x86, boolean is32Bit, EmulatorControl disciple, int[] input, int[] discipleState,
        int[] oracle, int flagMask, BufferedWriter log) throws IOException {
        ...
        for (Integer index : diff)
            log.write(String.format("Difference: %s %08x - %08x : ^ %08x\n", EmulatorControl.names[index], discipleState[index],
                oracle[index], discipleState[index] ^ oracle[index]));
        log.write("Input:\n");
